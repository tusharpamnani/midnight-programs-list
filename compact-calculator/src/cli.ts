/**
 * CLI for interacting with compact-calculator contract
 */
...
const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'calculator');
...
const Calculator = await import(pathToFileURL(contractPath).href);

const compiledContract = CompiledContract.make('calculator', Calculator.Contract).pipe(
...
async function createProviders(walletCtx: ReturnType<typeof createWallet> extends Promise<infer T> ? T : never) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({ privateStateStoreName: 'calculator-state', walletProvider }),
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   compact-calculator CLI                           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    const deployed: any = await findDeployedContract(providers, {
      ...
      privateStateId: 'calculatorState',
      initialPrivateState: {},
    });
    ...
    while (running) {
      ...
      console.log('  5. Divide (with witness & on-chain verification)');
      console.log('  6. Read current result');
      ...
      switch (choice.trim()) {
        case '1': {
          const num1 = parseInt(await rl.question('  Enter first number: '));
          const num2 = parseInt(await rl.question('  Enter second number: '));
          console.log('\n  Submitting transaction...');
          try {
            const tx = await deployed.callTx.add(num1, num2);
        ...
        case '2': {
          ...
          console.log('\n  Submitting transaction...');
          try {
            const tx = await deployed.callTx.subtract(num1, num2);
            console.log(`\n  ✅ Transaction successful!`);
            console.log(`  Transaction ID: ${tx.public.txId}\n`);
          } catch (error) {
            console.error('\n  ❌ Failed:', error instanceof Error ? error.message : error);
        ...
        case '3': {
          ...
          console.log('\n  Submitting transaction...');
          try {
            const tx = await deployed.callTx.multiply(num1, num2);
            console.log(`\n  ✅ Transaction successful!`);
            console.log(`  Transaction ID: ${tx.public.txId}\n`);
          } catch (error) {
            console.error('\n  ❌ Failed:', error instanceof Error ? error.message : error);
        ...
        case '4': {
          const num = parseInt(await rl.question('  Enter number to square: '));
          console.log('\n  Submitting transaction...');
          try {
            const tx = await deployed.callTx.square(num);
            console.log(`\n  ✅ Transaction successful!`);
            console.log(`  Transaction ID: ${tx.public.txId}\n`);
          } catch (error) {
            console.error('\n  ❌ Failed:', error instanceof Error ? error.message : error);
        ...
        case '5': {
          const num1 = parseInt(await rl.question('  Enter dividend: '));
          const num2 = parseInt(await rl.question('  Enter divisor: '));
          console.log('\n  Submitting transaction...');
          try {
            // Note: divide circuit uses the divMod witness which we need to provide
            // However, CompiledContract.withVacantWitnesses will use default witnesses if not provided.
            // In a real app, you'd provide the implementation in the providers.
            // For this boilerplate, we'll assume the environment/Boilerplate handles basic witnesses 
            // or we'd need to add them to provideWitnesses.
            const tx = await deployed.callTx.divide(num1, num2);
            console.log(`\n  ✅ Transaction successful!`);
            console.log(`  Transaction ID: ${tx.public.txId}\n`);
          } catch (error) {
            console.error('\n  ❌ Failed:', error instanceof Error ? error.message : error);
        ...
        case '6': {
          console.log('\n  Reading result from blockchain...');
          try {
            const contractState = await providers.publicDataProvider.queryContractState(deployment.contractAddress);
            if (contractState) {
              const ledgerState = Calculator.ledger(contractState.data);
              console.log(`\n  📋 Current result: ${ledgerState.result}\n`);
            } else {
              console.log('\n  📋 No result found\n');
            }
          } catch (error) {
