...
type CoinFlipModule = typeof import('../contracts/managed/coinflip/contract/index.js');
type CoinFlipContract = CompactContract<undefined>;

const coinFlipModulePromise = (async () => {
  return import(pathToFileURL(path.join(zkConfigPath, 'contract', 'index.js')).href) as Promise<CoinFlipModule>;
})();

export const CoinFlip = await coinFlipModulePromise;

const CoinFlipContractCtor =
  CoinFlip.Contract as unknown as new (
    witnesses: Record<string, never>,
  ) => CoinFlipContract;

export const compiledContract = CompiledContract.make(
  'coinflip',
  CoinFlipContractCtor,
).pipe(
...
export async function createProviders(
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'mn-coinflip-state',
      accountId,
...
export function computeHash(secret: Buffer): Buffer {
  return createHash('sha256').update(secret).digest();
}

export function getResult(secret: Buffer): 'HEADS' | 'TAILS' {
  // Use the last byte like in the contract logic (simplified)
  return secret[secret.length - 1] % 2 === 0 ? 'HEADS' : 'TAILS';
}
