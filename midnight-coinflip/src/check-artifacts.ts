...
export function ensureCompiledArtifacts() {
  if (!fs.existsSync(contractSourcePath)) {
    throw new Error(
      'Missing contracts/coinflip.compact. Restore the contract source before compiling.',
    );
  }

  if (!fs.existsSync(generatedContractPath)) {
    throw new Error(
      'Missing compiled contract artifacts. Run `npm run compile` before `npm run build`, `npm run deploy`, or `npm run play`.',
    );
  }
}
